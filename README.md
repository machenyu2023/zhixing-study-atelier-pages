...
面向个人长期学习的教材阅读、刷题、思辨、写作与复盘网站。目前覆盖雅思、逻辑、写作和数学，内置 1980 道题，并完整收录《系统学习手册》13 章和《每日训练习题册》使用指南及 6 卷正文。其中雅思题共 425 道、逻辑题共 452 道、数学题共 1044 道，数学重点为优化、矩阵论、随机过程和高等概率论。

## 核心能力

- 遥感物理个人知识库：十二板块知识地图、三个入门条目、标准十项条目编辑、全文搜索、个人笔记与自评、可追溯来源、JSON 合并导入和 Markdown 导出。
- Planck 黑体实验：温度滑块、固定尺度光谱对照、CSV 导出与仅依赖标准库的 Python Demo。
- 遥感内容维护说明见 [REMOTE_SENSING.md](docs/REMOTE_SENSING.md)。
...
- 400 道高阶数学详解题：四门各 100 道，含 160 道概念选择、160 道计算填空和 80 道证明推导；每题都有标准答案和分步解法。
- 818 道可即时判分的选择题。
- 925 道支持多答案和数值容差的填空题。
- 237 道材料阅读、证明推导、论证简答和写作开放题。
- 选择题解析、错题自动收集、重做和掌握归档。
- 开放题草稿、建议字数、五维自评和可选 AI 阅卷。
- 每组练习完成后生成结果页，并把整套题目、原始作答、网站初判、标准答案和评分依据导出为 AI 可直接读取的 Markdown 答卷；最近 30 套答卷保存在本机，可从“数据管理”重新下载。
- 统一成绩：每题先归一化为百分制，再计算平均分，避免不同题型权重失真。
- IndexedDB 本地存储，自动迁移旧版 `localStorage` 数据。
- JSON 题库批量导入、学习数据导出、整套答卷与开放题 Prompt 导出，以及 GitHub Pages 自动部署。
...
完成一组练习后，可在结果页点击“下载 AI 阅卷答卷”。下载的 `.md` 文件是自包含答卷，选择题、填空题和开放题可以混合导出；把文件上传给任意支持附件的 AI，即可按文件顶部的统一要求复核整卷。每道题先换算为百分制再等权平均，与网站现有综合分口径一致。此导出完全在浏览器本地生成，不需要 API Key，也不会上传个人作答。
...
内置扩展题库由 [generate-open-bank.mjs](scripts/generate-open-bank.mjs)、[generate-ielts-bank.mjs](scripts/generate-ielts-bank.mjs) 和 [generate-advanced-math-bank.mjs](scripts/generate-advanced-math-bank.mjs) 确定性生成。IELTS 题目的文章、transcript、题干、选项、答案和解析均为项目原创；官方链接只用于核对题型结构，并不表示官方认可。高阶数学详解题保存 `referenceAnswer` 和 `solutionSteps`，证明题完成后会显示标准推导。公开资源接入与抓取边界见 [OPEN_BANK_PIPELINE.md](docs/OPEN_BANK_PIPELINE.md)，版权和来源规则见 [COPYRIGHT.md](docs/COPYRIGHT.md)。
