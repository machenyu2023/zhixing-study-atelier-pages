...
- `referenceAnswer`: 简明、可直接核对的最终结论；
- `solutionSteps`: 至少两项分步推导；
- `minimumResponseUnits`: 开放型数学题允许提交所需的最少文本单位，建议为 `5`。

选择题和填空题交卷后会展开标准答案；开放题完成后会在自评区显示参考推导。`data/question-banks/advanced-math-solution-bank.json` 是完整示例。
