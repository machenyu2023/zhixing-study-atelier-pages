...
const RemoteSensing = (() => {
  ...
  const SECTION_CONTEXT = {
    "01":"任何遥感量都先是能量和几何问题。我们要问的是：什么能量到达了哪里，来自哪个方向，经过了哪些面积、立体角和波段积分。这个层次决定量纲，也决定后面能否把不同传感器的数字放在一起比较。先从守恒、投影和积分建立语言，再进入具体波段。",
    "02":"电磁波谱是连续的，而卫星只用有限的波段采样它。波长、频率、带宽和响应函数共同决定了传感器真正看到的谱信息；因此一个‘波段反射率’本质上是仪器加权后的观测，不是自然界给出的单一标签。",
    "03":"辐射在介质中传播时会被吸收、散射或改变方向。辐射传输方程把这些过程写成沿路径的局部收支，是连接分子性质、大气状态和传感器信号的桥梁。光学厚度、透过率和路径辐射必须一起读。",
    "04":"太阳反射遥感先测到 TOA radiance，再借助太阳几何、大气状态和传感器定标解释地表。TOA reflectance 只是几何归一化，surface reflectance 还需要估计大气影响；两者服务于不同的比较和应用。",
    "05":"地表反射并不只由‘材料名称’决定，还取决于入射方向、出射方向、粗糙度、阴影和尺度。BRDF 将方向性显式化，帮助我们理解为什么同一地物在不同观测角度下会有不同亮度，并为方向归一化提供物理基础。",
    "06":"热红外传感器接收的是热辐射亮度。Planck 定律给出温度与谱辐射的关系，但真实地表还具有小于一的发射率，并受到大气上、下行辐射影响，所以亮温、辐射温度和物理温度必须分开。",
    "07":"微波波长较长，能够穿透部分云层和植被，并对介电常数、粗糙度和含水量敏感。主动系统测回波，被动系统测自然辐射；极化和入射角改变电场与表面的耦合方式，是解释信号差异的关键。",
    "08":"土壤水分不是微波信号的唯一控制量。含水量改变介电常数，介电常数又改变发射率和散射；温度、质地、粗糙度、植被和冻结状态会同时进入前向模型。可辨识性必须通过多频率、多极化或先验约束来改善。",
    "09":"冠层观测是叶片光学性质、叶面积、叶倾角、间隙率和背景共同作用的结果。Beer 定律只描述未碰撞光束；完整冠层反射需要考虑多次散射、热点、土壤背景和尺度效应，植被指数因此是有条件的代理量。",
    "10":"前向模型从一组状态和观测条件生成传感器信号，是检验物理假设的正向实验。写模型时要区分状态、参数、边界条件和仪器响应；把 DN、radiance、reflectance 或 Tb 混为一个 y 会掩盖误差来源。",
    "11":"反演是信息不足条件下的推断。优化器只能在观测和先验允许的范围内选择解，不能消除不可辨识性。评价反演应同时看残差、独立验证、参数相关性、误差协方差以及跨地表和跨季节迁移。",
    "12":"卫星产品是测量、定标、算法和质量控制的组合结果。读取产品时要查波段响应、定标系数、饱和与噪声、空间足迹、重采样和 QA 位；产品名称相同并不意味着物理量和处理级别相同。"
  };
